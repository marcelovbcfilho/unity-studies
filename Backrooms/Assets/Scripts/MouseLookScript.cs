...
public class MouseLookScript : MonoBehaviour
{
    ...
    [HideInInspector] public float mouseSensitvity_notAiming = 300;

    [HideInInspector] public float mouseSensitvity_aiming = 50;
    ...
    [Tooltip("Current weapon that player carries.")] [HideInInspector]
 ...
 */
    private void ApplyingStuff()
    {
        currentYRotation = Mathf.SmoothDamp(currentYRotation, wantedYRotation, ref rotationYVelocity, yRotationSpeed);
        currentCameraXRotation = Mathf.SmoothDamp(currentCameraXRotation, wantedCameraXRotation, ref cameraXVelocity,
            xCameraSpeed);
        ...
        if (!weapon)
        {
            ...
            if (weapon)
                if (weapon.GetComponent<GunScript>())
                    ...
                    {
                        print("gun not found->" + ex.StackTrace);
                    }
        }
    }
